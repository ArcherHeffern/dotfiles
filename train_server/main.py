...
LAT = 42.36341288914495
LNG = -71.26184926824149
# --- CONFIGURATION ---
...
def find_nearby_train_stations():
    ...
    try:
        response = requests.post(
            PLACES_API_URL, headers=headers, data=json.dumps(payload), timeout=10
        )
...
def get_route_info(destination_location):
    ...
    try:
        response = requests.post(
            ROUTES_API_URL, headers=headers, data=json.dumps(payload), timeout=10
        )
...
def m():
    ...
    if API_KEY == "YOUR_API_KEY":
        print(
            "Error: Please replace 'YOUR_API_KEY' with your actual Google Maps Platform API key."
        )
        return
